...
const PHASES = [
  ...
  },
  {
    ...
    label: "PHASE 02",
    headline: "Audit",
    body: "MemWal recalls similar past findings from Walrus Memory. Parallel agents scan for capability leakage, ownership bugs, hot-potato misuse, unsafe upgrades, missing signer checks, and overflow risk — benchmarked against OpenZeppelin's audited math library.",
  },
  {
    icon: "/S3.png",
    ...
    body: "Findings are structured and stored back to Walrus Memory for future recall. Seal encrypts the full report — the contract owner gets a private draft first, before anyone else.",
  },
  {
    icon: "/S4.png",
...
export function HowItWorks() {
  ...
  // Decide once on mount (and on resize/media changes) whether to run the
  // pinned scroll mechanic. Defaults to the static fallback to avoid any
  // SSR/hydration mismatch — only opts into pinning client-side.
  useEffect(() => {
  ...
  // Scroll progress through the 400vh container, banded into 4 discrete
  // steps. No wheel-jacking / preventDefault — this only reads scroll
  // position and lets native scroll physics run untouched.
  useEffect(() => {
...
function Card({
  ...
  // DOM order is text-then-illustration, so odd-numbered phases (index 0, 2 ->
  // Phase 01, 03) need no reverse to get text-left/illustration-right; even
  // phases (index 1, 3 -> Phase 02, 04) need the reverse to flip to
