  ...
  return (
    <section className="relative w-full bg-black px-6 pt-8 sm:pt-10 pb-16 sm:pb-20">
      ...
      <div className="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-5 lg:gap-6">
        {/* Card 1: Seal encryption */}
        <div className="lg:row-span-2 rounded-[24px] flex flex-col p-8 sm:p-10 lg:p-12" style={CARD_STYLE}>
          ...
          <div className="mt-10 grid grid-cols-3 gap-3">
            {LOCK_STATES.map((state) => (
              <div key={state.label} className="flex flex-col gap-1.5 rounded-[14px] px-4 py-4" style={{ background: "#CAB1FF", border: "1px solid rgba(202,177,255,0.4)" }}>
                <span className="font-display font-bold text-white" style={{ fontSize: 15, lineHeight: 1.2 }}>{state.label}</span>
                <span className="font-sans-switzer" style={{ fontSize: 12, lineHeight: 1.45, color: "rgba(255,255,255,0.45)" }}>{state.sub}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Card 2: LanceDB semantic recall */}
        <div className="rounded-[24px] flex flex-col justify-between p-7 sm:p-8" style={CARD_STYLE}>
          <div>
          ...
          <p className="text-[14px] sm:text-[15px] leading-[1.6] text-[var(--text-secondary)] font-sans-switzer">
            LanceDB semantic recall finds similar patterns from 52 known exploit snippets before flagging anything new. Every audit sharpens the next one.
          </p>
        </div>
